...
def dashboard_view(request):
    ...
    today_sessions = Session.objects.filter(date__date=today, course__in=courses)
    tomorrow = today + timedelta(days=1)
    tomorrow_sessions = Session.objects.filter(date__date=tomorrow, course__in=courses)

    today_sessions_with_end_time = [(session, session.date + session.duration) for session in today_sessions]
    tomorrow_sessions_with_end_time = [(session, session.date + session.duration) for session in tomorrow_sessions]
    ...
    future_exams = Session.objects.filter(date__gt=timezone.now(), exam=True, course__in=courses).distinct().order_by('date')[:3]

    context = {
        'user': user,
        'today_sessions': today_sessions_with_end_time,
        'tomorrow_sessions': tomorrow_sessions_with_end_time,  # Utilisez la liste modifiée avec l'heure de fin
