...
def attendance_student(request):
    user = request.user
    absences = Presence.objects.filter(user=user, presence="absent", justified=False).order_by('-session__date')
    lates = Presence.objects.filter(user=user, presence="late", justified=False).order_by('-session__date')
